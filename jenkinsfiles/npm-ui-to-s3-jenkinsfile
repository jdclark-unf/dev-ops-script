pipeline {
    ...
    stages {
      ...
      // the build can often cause jenkins to crash if it is only an ec2 micro due to running out of memory
      stage('NPM Build') {
          steps {
              // go into client-side directory
              dir('week4') {
          ...
          steps {
              dir('week4') {
                  dir('redux-demo') {
                      ...
                      // you will need the s3 plugin for this to work, I prefer using aws codebuild and codepipeline for automating this task
